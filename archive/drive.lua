...
if _Tune.AutoStart and (_Tune.Engine or _Tune.Electric) then
...
function DealWithInput(input, IsRobloxFunction)
	if (UserInputService:GetFocusedTextBox() == nil) and not _InControls then --Ignore when UI Focus
		...
		if
			(
				input.KeyCode == _CTRL["ContlrShiftDown"]
				or (_MSteer and input.KeyCode == _CTRL["MouseShiftDown"])
				or ((not _MSteer) and input.KeyCode == _CTRL["ShiftDown"])
			)
			and ((_IsOn and ((_TMode == "Auto" and _CGear <= 1) and _Tune.AutoShiftVers == "New") or _TMode == "Semi") or _TMode == "Manual")
		...
		elseif
			(
				input.KeyCode == _CTRL["ContlrShiftUp"]
				or (_MSteer and input.KeyCode == _CTRL["MouseShiftUp"])
				or ((not _MSteer) and input.KeyCode == _CTRL["ShiftUp"])
			)
			and ((_IsOn and ((_TMode == "Auto" and _CGear < 1) and _Tune.AutoShiftVers == "New") or _TMode == "Semi") or _TMode == "Manual")
			and input.UserInputState == Enum.UserInputState.Begin
		then
		...
		elseif
			(
				input.KeyCode == _CTRL["ContlrClutch"]
				or (_MSteer and input.KeyCode == _CTRL["MouseClutch"])
				or ((not _MSteer) and input.KeyCode == _CTRL["Clutch"])
			) and _TMode == "Manual"
		then
			...
			--Toggle PBrake
		elseif
			input.KeyCode == _CTRL["ContlrPBrake"]
		...
		then
			...
			--Throttle
		elseif
			((not _MSteer) and (input.KeyCode == _CTRL["Throttle"] or input.KeyCode == _CTRL["Throttle2"]))
			or (
				(
					(
						(
...
for i, v in pairs(car.Wheels:GetChildren()) do
	if
		(math.abs(v["#BV"].MotorMaxTorque - PBrakeForceF) < 1) or (math.abs(v["#BV"].MotorMaxTorque - PBrakeForceR) < 1)
	then
		_PBrake = true
	end
end
	...
	for i, v in pairs(car.Wheels:GetChildren()) do
		if v.Name == "F" then
			v.Arm.Steer.CFrame = car.Wheels.F.Base.CFrame
				* CFrame.Angles(0, -math.rad(_GSteerC * _Tune.SteerInner * sDecay), 0)
		elseif v.Name == "FL" then
			if _GSteerC >= 0 then
				v.Arm.Steer.CFrame = car.Wheels.FL.Base.CFrame
					* CFrame.Angles(0, -math.rad(_GSteerC * SteerOuter * sDecay), 0) --luaint edit
			else
				v.Arm.Steer.CFrame = car.Wheels.FL.Base.CFrame
					* CFrame.Angles(0, -math.rad(_GSteerC * SteerInner * sDecay), 0) --luaint edit
			end
		...
		elseif v.Name == "R" then
			...
			elseif _Tune.FWSteer == "Static" then
				v.Arm.Steer.CFrame = car.Wheels.R.Base.CFrame
					* CFrame.Angles(
						0,
						math.rad(
							_GSteerC
								* RSteerInner
								* rsDecay
								* math.max(0, 1 - (car.DriveSeat.Velocity.Magnitude / _Tune.RSteerSpeed))
						),
						0
					) --luaint edit
					...
					* CFrame.Angles(
						...
						-math.rad(
							_GSteerC
								* RSteerInner
								* rsDecay
								* math.min(1, (car.DriveSeat.Velocity.Magnitude / _Tune.RSteerSpeed))
						),
						0
					) --luaint edit
					...
					* CFrame.Angles(
						...
						math.rad(
							_GSteerC
								* RSteerInner
								* rsDecay
								* math.max(-1, 1 - (car.DriveSeat.Velocity.Magnitude / _Tune.RSteerSpeed))
						),
						0
					) --luaint edit
			end
		elseif v.Name == "RL" then
			...
			elseif _Tune.FWSteer == "Static" then
				if _GSteerC >= 0 then
					v.Arm.Steer.CFrame = car.Wheels.RL.Base.CFrame
						* CFrame.Angles(
							0,
							math.rad(
								_GSteerC
		...
		elseif v.Name == "RR" then
			...
			elseif _Tune.FWSteer == "Static" then
				if _GSteerC >= 0 then
					v.Arm.Steer.CFrame = car.Wheels.RR.Base.CFrame
						* CFrame.Angles(
							0,
							math.rad(
								_GSteerC
...
function elecHP(RPM)
	...
	local retVal = 1e-9
	if RPM <= ETrans1 then
		retVal = ((((RPM / ETrans1) ^ _Tune.EH_FrontMult) / (1 / EHP)) * (RPM / ETrans1))
			+ ((((RPM / ETrans1) ^ (1 / _Tune.EH_FrontMult)) / (1 / EHP)) * (1 - (RPM / ETrans1)))
	...
	elseif ETrans2 <= RPM then
		...
		error(
			"\n\t [AC6C]: Drive initialization failed!"
				...
				.. "\n\t    Please send a screenshot of this message to pointclouded."
				.. "\n\t    R: "
				.. RPM
				.. ", T1: "
				.. ETrans1(", T2: ")
...
function Auto()
	...
	if _IsOn then
		...
		if _CGear >= 1 then
			if
				(_CGear == 1 and _InBrake > 0 and car.DriveSeat.Velocity.Magnitude < 5)
				and _Tune.AutoShiftVers == "Old"
			then
				_CGear = -1
				_ClPressing = false
				...
				if _Tune.AutoShiftMode == "RPM" then
					...
					elseif
						math.max(
							...
							< (_Tune.PeakRPM - _Tune.AutoDownThresh)
						and _CGear > 1
					then
					...
					if
						car.DriveSeat.Velocity.Magnitude
						> math.ceil(wDRatio * (_Tune.PeakRPM + _Tune.AutoUpThresh) / _Tune.Ratios[_CGear + 2] / fFD)
					then
						if not _ShiftUp and not _Shifting then
							_ShiftUp = true
						end
					elseif
						car.DriveSeat.Velocity.Magnitude
							< math.ceil(
								wDRatio * (_Tune.PeakRPM - _Tune.AutoDownThresh) / _Tune.Ratios[_CGear + 1] / fFD
							)
						and _CGear > 1
					then
						if not _ShiftDn and not _Shifting then
			...
			if
				(_InThrot - (_Tune.IdleThrottle / 100) > 0 and car.DriveSeat.Velocity.Magnitude < 5)
				and _Tune.AutoShiftVers == "Old"
			then
				_CGear = 1
				_ClPressing = false
...
function Gear()
	...
	if _ShiftUp and not _Shifting then
		if
			(_TMode == "Manual" and not _ClPressing)
			or (_TMode == "Manual" and _Tune.ClutchRel and (_InThrot - (_Tune.IdleThrottle / 100) > 0))
			or _CGear == #_Tune.Ratios - 2
			or (_TMode ~= "Manual" and not _IsOn)
		then
			_ShiftUp = false
			return
		end
		...
		if _TMode ~= "Manual" then
			...
			if _CGear > 0 then
				...
				elseif _Tune.AutoShiftType == "Rev" then
					...
					until _RPM
							<= math.max(
								math.min(maxSpin * _Tune.Ratios[NextGear] * fFDr, _Tune.Redline - _Tune.RevBounce),
								_Tune.IdleRPM
							)
						or not _IsOn
						or _ShiftDn
		...
		if _TMode ~= "Manual" then
			...
			if _CGear > 1 then
				...
				elseif _Tune.AutoShiftType == "Rev" then
					...
					until _RPM
							>= math.max(
								math.min(maxSpin * _Tune.Ratios[PrevGear] * fFDr, _Tune.Redline - _Tune.RevBounce),
								_Tune.IdleRPM
							)
						or not _IsOn
						or _ShiftUp
...
function Engine(dt)
	...
	if _Tune.Stall and _Tune.Clutch then
		revMin = 0
	end
	...
	if _Tune.Engine or _Tune.Electric then
		_GoalRPM = math.clamp(
			(_RPM - _Tune.RevDecay * deltaTime) + ((_Tune.RevDecay + _Tune.RevAccel) * revThrot * deltaTime),
			goalMin,
			_Tune.Redline + 100
		)
	end
	...
	if _GoalRPM > _Tune.Redline then
		if _CGear < #_Tune.Ratios - 2 then
			...
			_GoalRPM = _GoalRPM - _Tune.RevBounce * 0.5
		end
	end
	...
	if _Tune.Clutch then
		if script.Parent.Values.AutoClutch.Value and _IsOn then
			if _Tune.ClutchType == "Clutch" then
				...
				local ClRPMInfluence =
					math.max((_RPM - _Tune.IdleRPM) * _Tune.ClutchRPMMult / (_Tune.Redline - _Tune.IdleRPM), 0)
				...
				_ClutchModulate = math.min(
					(
						(
							((script.Parent.Values.Velocity.Value.Magnitude / _Tune.SpeedEngage) / math.abs(_CGear))
							+ ClRPMInfluence
						) - _ClutchKick
					),
					1
			...
			elseif _Tune.ClutchType == "CVT" or (_Tune.ClutchType == "TorqueConverter" and _Tune.TQLock) then
				if
					(
						_GThrot - (_Tune.IdleThrottle / 100) == 0
						and script.Parent.Values.Velocity.Value.Magnitude < _Tune.SpeedEngage
					)
					or (
						_GThrot - (_Tune.IdleThrottle / 100) ~= 0
						and (_RPM < _Tune.RPMEngage and _WheelRPM < _Tune.RPMEngage)
					)
				then
					_ClutchModulate = math.min(_ClutchModulate * (_Tune.ClutchEngage / 100), 1)
				else
					_ClutchModulate =
						math.min(_ClutchModulate * (_Tune.ClutchEngage / 100) + (1 - (_Tune.ClutchEngage / 100)), 1)
				end
			elseif _Tune.ClutchType == "TorqueConverter" and not _Tune.TQLock then
				_ClutchModulate = math.min((_RPM / _Tune.Redline) * 0.7, 1)
			end
			if not _ClPressing then
		...
		_StallOK = false
		if not _ClPressing and not _Shifting then
			_Clutch = 0
		else
			_Clutch = 1
		end
	...
	if _Tune.Engine then
		if _TCount ~= 0 and _TCount ~= 2 then
			_TBoost = _TBoost
				+ (
					(
						(
							(((_HP * (_BThrot * 1.2) / _Tune.Horsepower) / 8) - (_TBoost / TPsi * (TPsi / 15)))
							* ((8 / (_Tune.T_BoostLag / deltaTime)) * 2)
						) / TPsi
					) * 15
				)
			if _TBoost < 0.05 then
				_TBoost = 0.05
			elseif _TBoost > 2 then
				_TBoost = 2
			end
				...
				_TBoost = _TBoost
					+ (
						(
							(
								(((_HP * (_BThrot * 1.2) / _Tune.Horsepower) / 8) - (_TBoost / TPsi * (TPsi / 15)))
								* ((8 / (_Tune.T2_BoostLag / deltaTime)) * 2)
							) / TPsi
						) * 15
					)
			end
		...
		if _SCount ~= 0 then
			...
			_SBoost = (_RPM / _Tune.Redline) * (0.5 + (1.5 * sthrot))
		else
			_SBoost = 0
		end
	else
	...
	for i, v in pairs(car.Wheels:GetChildren()) do
		...
		if _Tune.DifferentialType == "Old" then
				...
				diffMult = math.max(
					...
					math.min(
						...
						1
							+ (
								(
									((v.RotVelocity.Magnitude - fwspeed) / fwspeed)
									/ (math.max(_Tune.FDiffSlipThres, 1) / 100)
								) * ((_Tune.FDiffLockThres - 50) / 50)
							)
					)
				)
						...
						math.min(
							...
							diffMult
								* (
									1
									+ (
										(((fwspeed - cwspeed) / cwspeed) / (math.max(_Tune.CDiffSlipThres, 1) / 100))
										* ((_Tune.CDiffLockThres - 50) / 50)
									)
								)
						)
					)
				...
				diffMult = math.max(
					...
					math.min(
						...
						1
							+ (
								(
									((v.RotVelocity.Magnitude - rwspeed) / rwspeed)
									/ (math.max(_Tune.RDiffSlipThres, 1) / 100)
								) * ((_Tune.RDiffLockThres - 50) / 50)
							)
					)
				)
				if _Tune.Config == "AWD" then
						...
						math.min(
							...
							diffMult
								* (
									1
									+ (
										(((rwspeed - cwspeed) / cwspeed) / (math.max(_Tune.CDiffSlipThres, 1) / 100))
										* ((_Tune.CDiffLockThres - 50) / 50)
									)
								)
						)
			...
			if v.Name == "FR" then
				local avg = ((v.RotVelocity.Magnitude / car.Wheels.FL.RotVelocity.Magnitude) - 1)
					* (_Tune.FDiffPreload / 10)
				FRdiffMult = math.ceil(
					math.max(
						...
						math.min(
							2,
							1 - (
									((_Tune.FDiffPower / 100) * avg * _GThrot) + (
										(_Tune.FDiffCoast / 100)
										* avg
										* (1 - _GThrot)
									)
								)
			...
			elseif v.Name == "FL" then
				local avg = ((v.RotVelocity.Magnitude / car.Wheels.FR.RotVelocity.Magnitude) - 1)
					* (_Tune.FDiffPreload / 10)
			...
			elseif v.Name == "RR" then
				local avg = ((v.RotVelocity.Magnitude / car.Wheels.RL.RotVelocity.Magnitude) - 1)
					* (_Tune.RDiffPreload / 10)
				RRdiffMult = math.ceil(
					math.max(
						...
						math.min(
							2,
							1 - (
									((_Tune.RDiffPower / 100) * avg * _GThrot) + (
										(_Tune.RDiffCoast / 100)
										* avg
										* (1 - _GThrot)
									)
								)
			...
			elseif v.Name == "RL" then
				local avg = ((v.RotVelocity.Magnitude / car.Wheels.RR.RotVelocity.Magnitude) - 1)
					* (_Tune.RDiffPreload / 10)
		...
		if
			_ABS
			and brake > 0
			and math.abs(v.RotVelocity.Magnitude * (v.Size.x / 2) - v.Velocity.Magnitude) - _Tune.ABSThreshold > 0
		then
			tqABS = 0
		end
		_ABSActive = (tqABS < 1)
		...
		if driven then
				...
				tqTCS = 1
					- (
						math.min(
							math.max(
								0,
								math.abs(v.RotVelocity.Magnitude * (v.Size.x / 2) - v.Velocity.Magnitude)
									- _Tune.TCSThreshold
							) / _Tune.TCSGradient,
							1
						) * (1 - (_Tune.TCSLimit / 100))
					)
			end
			if tqTCS < 1 then
				_TCSAmt = tqTCS
			...
			if
				_Tune.ClutchKick
				and car.DriveSeat.Velocity.Magnitude < _Tune.KickSpeedThreshold
				and _RPM > _Tune.Redline - _Tune.KickRPMThreshold
				and v["#BV"].MotorMaxTorque < 1
			then
			...
			if string.find(v.Name, "F") then
				v["#BV"].MotorMaxTorque = (FBrakeForce * (60 / workspace:GetRealPhysicsFPS()) * brake * tqABS)
					+ (EBrakeForce * ((1 - throt) * (_RPM / _Tune.Redline)))
					+ (PBrakeForceF * PBrakeV)
			else
				v["#BV"].MotorMaxTorque = (RBrakeForce * (60 / workspace:GetRealPhysicsFPS()) * brake * tqABS)
					+ (EBrakeForce * ((1 - throt) * (_RPM / _Tune.Redline)))
					+ (PBrakeForceR * PBrakeV)
			end
		else --luaint edit
...
print("AC6C V" .. ver .. " Loaded, Update " .. script.Parent.Version.Value)
...
game["Run Service"].Heartbeat:connect(function(dt)
	...
	script.Parent.Values.Torque.Value = script.Parent.Values.TqNatural.Value
		+ script.Parent.Values.TqElectric.Value
		+ script.Parent.Values.TqBoosted.Value
